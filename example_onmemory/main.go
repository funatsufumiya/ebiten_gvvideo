...
func (g *Game) toggleAsync() {
	g.async = !g.async
	g.player.Stop()
	player, err := gvplayer.LoadGVPlayerOnMemoryWithOption(g.gvPath, g.async)
	if err != nil {
		g.err = err
		return
	}
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		fmt.Println("[INFO] Playing the default GV video (on-memory). You can specify a .gv file as an argument.")
	}
	player, err := gvplayer.LoadGVPlayerOnMemoryWithOption(gvPath, true)
	if err != nil {
		log.Fatal(err)
	}
	...
	ebiten.SetWindowTitle("GV Video OnMemory (Ebitengine Demo)")
